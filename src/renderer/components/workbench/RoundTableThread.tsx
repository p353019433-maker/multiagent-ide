...
/** Subtle tint per role so cards are visually distinguishable at a glance. */
const ROLE_TINT: Record<AgentRole, { bg: string; chip: string; chipText: string }> = {
  architect: { bg: '#f6f3ec', chip: '#e8dfc4', chipText: '#7a5d20' },
  security: { bg: '#f9efee', chip: '#ecd2cf', chipText: '#8a2e2e' },
  testing: { bg: '#eef3f6', chip: '#cdd9e3', chipText: '#1f4a63' },
  style: { bg: '#f2eef6', chip: '#d6cbe3', chipText: '#4a2363' },
  general: { bg: '#f4f4f2', chip: '#e3e3df', chipText: '#3a3a3a' },
};
...
/**
 * Center column for round mode: stage indicator, topic card, parallel review
 * cards (one per role) and the moderator's synthesized-plan card with the
 * "让 agent 实现" trigger. Driven by the shared useRoundTable() instance.
 */
export default function RoundTableThread({ rt, onConfigure }: { rt: RoundTableState; onConfigure?: () => void }) {
  ...
  const okCards = rt.cards.filter((c) => c.ok);

  // 'pending' before any activity; 'active' while review runs; 'done' once
  // cards exist or the run finished. Annotated so the Stage prop type stays
  // narrow without a cast at the call site.
  const reviewState: 'done' | 'active' | 'pending' =
    okCards.length > 0 || rt.running
      ? rt.plan || !rt.running
        ? 'done'
        : 'active'
      : 'pending';
  ...
  // Collapse the weight table into a per-agent total for a compact "可信度" badge.
  const weightTotals = useMemo(() => {
    const out: Record<string, number> = {};
    for (const [id, w] of Object.entries(rt.weights)) {
      const sum = (w.architect || 0) + (w.security || 0) + (w.testing || 0) + (w.style || 0) + (w.general || 0);
      out[id] = Math.round(sum * 100) / 100;
    }
    return out;
  }, [rt.weights]);
    ...
    <div className="flex min-w-0 flex-1 flex-col bg-background">
      ...
      <div className="flex h-[54px] flex-none items-center justify-between border-b border-border px-6">
        <div className="flex min-w-0 items-center gap-2.5">
          <span className="truncate text-[14.5px] font-semibold text-foreground">圆桌评审</span>
          <span className="flex flex-none items-center gap-[7px]">
            <Stage label="评审" state={reviewState} />
            <span className="text-foreground/20">›</span>
            <Stage label="整合" state={convergeState} />
            <span className="text-foreground/20">›</span>
      ...
      <div className="flex flex-none items-center gap-2.5 border-b border-border/70 px-6 py-2" style={{ background: '#fbfbfa' }}>
        ...
        <span className="text-xs font-semibold text-foreground">
          {rt.phase || (rt.plan ? '已整合统一方案' : rt.enabled.length === 0 ? '未启用智能体' : '准备评审')}
        </span>
        <span className="ml-auto flex flex-none gap-3.5 whitespace-nowrap font-mono text-[10.5px] text-foreground/50">
          ...
          <span><b className="font-semibold text-foreground">{okCards.length}</b> 评审</span>
        </span>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-1 pt-5 selectable">
        <div className="mx-auto flex max-w-[760px] flex-col gap-4">
          {rt.enabled.length === 0 && rt.cards.length === 0 && (
            <div className="mt-8 flex flex-col items-center gap-3 text-center">
              ...
              <div className="text-sm font-semibold text-foreground">还没有可参与评审的智能体</div>
              <p className="max-w-[420px] text-[13px] leading-relaxed text-foreground/50">
                圆桌需要至少一个启用的智能体。配置好后，在左侧名册启用它们、并在「智能体」设置里为每个分配评审角色，再在下方提出议题——它们会从各自维度并行评审，主持人整合成统一方案，并各自在 worktree 实现。
              </p>
              {onConfigure && (
                <button
                  onClick={onConfigure}
          ...
          {rt.cards.length > 0 && (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {rt.cards.map((c) => {
                const v = agentVisual(kindById.get(c.agentId) ?? 'api');
                const tint = ROLE_TINT[c.role];
                return (
                  <div
                    key={`${c.agentId}-${c.role}`}
                    className="overflow-hidden rounded-[13px] border border-border shadow-card"
                  ...
                  >
                    <div className="flex items-center gap-2 border-b border-black/5 px-3.5 py-2.5">
                      ...
                      >
                        <v.Icon size={12} strokeWidth={1.8} style={{ color: v.iconColor }} />
                      </span>
                      <span className="truncate text-[12px] font-semibold text-foreground">{c.agentName}</span>
                      <span
                        className="ml-auto flex-none rounded font-mono text-[9px] font-semibold"
                        style={{ color: tint.chipText, background: tint.chip, padding: '1px 5px' }}
                      >
                        {ROLE_LABELS[c.role]}
                      </span>
                    </div>
                    <div className="px-3.5 py-3">
                      {c.ok ? (
                        <div className="whitespace-pre-wrap text-[13px] leading-[1.55] text-foreground/90">{c.text}</div>
                      ) : (
                        <div className="text-[12px] text-[#9a2533]">
                          评审失败{c.error ? `：${c.error}` : ''}
                        </div>
                      )}
                      {c.ok && weightTotals[c.agentId] !== undefined && (
                        <div className="mt-2 font-mono text-[10px] text-foreground/40">
                          可信度 {weightTotals[c.agentId].toFixed(2)} · {Math.round(c.durationMs / 1000)}s
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {rt.plan && (
            <div className="overflow-hidden rounded-[13px] border border-border shadow-card" style={{ background: '#fbfcf9' }}>
              <div className="flex items-center gap-2.5 border-b border-border/70 px-4 py-3">
                ...
                <span className="ml-auto text-[10.5px] text-foreground/40">综合 {okCards.length} 个评审</span>
              </div>
        ...
        <div className="mx-auto max-w-[760px]">
          ...
          <div className="rounded-[14px] border border-border-strong bg-background shadow-float transition-colors duration-150 focus-within:border-[hsl(0_0%_5%/0.18)]">
            <textarea
              ...
              rows={1}
              placeholder="提出议题或方案让智能体从各自维度评审…  回车直接发送"
