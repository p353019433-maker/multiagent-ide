    ...
    <div className="flex h-full flex-col bg-editor-sidebar border-l border-editor-border">
        ...
        {(['readonly', 'auto', 'full'] as ApprovalMode[]).map((m) => {
          ...
          return (
            <button
              ...
              title={meta.hint}
              className={`text-[10px] px-1.5 py-0.5 rounded-sm transition-colors duration-75 hover:bg-editor-active ${
                active
                  ? m === 'full'
            ...
            {checkpoints.slice(0, 5).map((cp) => (
              <div key={cp.id} className="flex items-center justify-between gap-2 text-[11px]">
                ...
                <button
                  ...
                  className="flex-shrink-0 px-1.5 py-0.5 rounded-sm bg-editor-hover text-gray-300 hover:bg-red-600 hover:text-white"
                  title="回滚此检查点的所有文件改动"
                >
                  ↩ 回滚
                </button>
              ...
              <div key={a.id} className="flex items-center justify-between gap-2 text-[11px]">
                ...
                {a.path && (
                  <button
                    ...
                    className="flex-shrink-0 px-1.5 py-0.5 rounded-sm bg-editor-hover text-gray-300 hover:bg-editor-accent hover:text-white"
                    title="打开交付报告"
                  >
                    查看
                  </button>
      ...
      {pendingApproval && pendingApproval.action !== 'edit' && pendingApproval.action !== 'write' && pendingApproval.action !== 'replace_in_file' && pendingApproval.action !== 'search_and_replace' ? (
        <div className={`px-3 py-2 border-t border-editor-border flex-shrink-0 ${pendingApproval.dangerReason ? 'bg-red-900/20' : 'bg-yellow-900/10'}`}>
          ...
          <pre className="text-[11px] text-gray-300 mt-1 whitespace-pre-wrap bg-black/20 rounded-sm p-2">
            {pendingApproval.after.slice(0, 500)}
          </pre>
          <div className="flex gap-2 mt-2">
            <button
              onClick={handleApprove}
              className="px-2 py-0.5 text-[11px] bg-green-600 text-white rounded-sm hover:bg-green-700"
            >
              接受
            </button>
            <button
              onClick={handleReject}
              className="px-2 py-0.5 text-[11px] bg-red-600 text-white rounded-sm hover:bg-red-700"
            >
              ✕ 拒绝
            </button>
          </div>
        ...
        <div className="h-[250px] border-t border-editor-border flex-shrink-0 relative">
          <div className="absolute top-2 right-2 z-10 flex items-center gap-2 bg-editor-sidebar/90 rounded-sm px-2 py-1 shadow">
            <span className="text-[11px] text-yellow-400 animate-pulse">
              {pendingApproval.countdown ? '5 秒后自动接受' : '需手动批准'}
            </span>
            <button
              onClick={handleApprove}
              className="px-1.5 py-0.5 text-[11px] bg-green-600 text-white rounded-sm hover:bg-green-700"
            >
              接受
            </button>
            <button
              onClick={handleReject}
              className="px-1.5 py-0.5 text-[11px] bg-red-600 text-white rounded-sm hover:bg-red-700"
            >
              ✕ 拒绝
            </button>
          </div>
          ...
          <>
                ...
                {pendingImages.map((img, i) => (
                  <div key={i} className="relative group">
                    <img
                      src={img}
                      alt="attachment"
                      className="h-12 w-12 object-cover rounded-sm border border-editor-border"
                    />
                    <button
            ...
            <div className="flex gap-2">
              <textarea
                ...
                className="flex-1 bg-editor-bg border border-editor-border rounded-sm px-3 py-2 text-sm text-editor-text resize-none outline-none focus:border-editor-accent transition-colors duration-75 hover:bg-editor-active"
                rows={2}
                disabled={isStreaming}
              />
              <div className="flex flex-col gap-1">
                <label
                  className="px-3 py-1 bg-editor-hover text-gray-300 text-xs rounded-sm hover:bg-editor-active transition-colors duration-75 hover:bg-editor-active cursor-pointer text-center"
                  title="附加图片"
                >
                  🖼
                  <input
                ...
                {isStreaming ? (
                  <button
                    onClick={handleAbort}
                    className="px-3 py-1 bg-red-600 text-white text-xs rounded-sm hover:bg-red-700 transition-colors duration-75 hover:bg-editor-active"
                  >
                    停止
                  </button>
                ) : (
                  <button
                    ...
                    className="px-3 py-1 bg-editor-accent text-white text-xs rounded-sm hover:opacity-90 transition-opacity disabled:opacity-40"
                  >
                    发送
                  </button>
                )}
